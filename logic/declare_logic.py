...
def declare_logic():
    ...
    # Derive the balance of a customer as the sum of order amounts where the order has not been shipped.
    Rule.sum(derive=Customer.balance, as_sum_of=Order.amount_total, where=lambda row: row.date_shipped is None)

    # Derive the total amount of an order as the sum of item amounts.
    Rule.sum(derive=Order.amount_total, as_sum_of=Item.amount)

    # Calculate item amount as a product of quantity and unit price.
    Rule.formula(derive=Item.amount, as_expression=lambda row: row.quantity * row.unit_price)

    # Copies the unit price from the Product table to the Item table.
    Rule.copy(derive=Item.unit_price, from_parent=Product.unit_price)

    # Prevent any customer from having a balance exceeding their credit limit.
